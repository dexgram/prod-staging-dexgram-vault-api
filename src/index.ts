...
async function getUser(db: D1Database, clientCode: string): Promise<UserRow | null> {
...
async function getAuthClientCode(request: Request, env: Env): Promise<string | null> {
...
async function verifyObjectAndReadHeaders(url: string): Promise<{ sizeBytes: number; contentType: string | null }> {
...
export default {
  async fetch(request, env, ctx): Promise<Response> {
    ...
    if (!env.SESSION_SECRET || !env.BUCKET_CONFIGS_JSON) {
      return badRequest("Server misconfigured", 500);
    }

    const buckets = parseBucketConfigs(env.BUCKET_CONFIGS_JSON);
    const url = new URL(request.url);

    if (request.method === "POST" && url.pathname === "/auth/login") {
      const body = (await request.json().catch(() => null)) as { clientCode?: string } | null;
      ...
      return json({ token, expiresInSeconds, ...usagePayload(user), subscriptionActive: isSubscriptionActive(user.subscription_expires_at) });
    }
    ...
    if (request.method === "POST" && url.pathname === "/uploads/request") {
      const body = (await request.json().catch(() => null)) as { mimeType?: string; sizeBytes?: number } | null;
    ...
    if (request.method === "POST" && url.pathname === "/uploads/complete") {
      ...
      const file = await env.DB
        .prepare("SELECT * FROM files WHERE file_id = ? AND client_code = ? AND deleted_at IS NULL")
      ...
      if (file.status === "active") {
        ...
        return json({ fileId: file.file_id, status: "active", ...usagePayload(refreshed ?? user) });
      }
      ...
      await env.DB.batch([
        env.DB
          .prepare("UPDATE files SET size_bytes = ?, mime_type = COALESCE(?, mime_type), status = 'active' WHERE file_id = ?")
      ...
      const refreshed = await getUser(env.DB, clientCode);
      return json({ fileId: file.file_id, sizeBytes: objectState.sizeBytes, ...usagePayload(refreshed ?? user) });
    }
    ...
    if (request.method === "GET" && downloadMatch) {
      ...
      const file = await env.DB
        .prepare("SELECT * FROM files WHERE file_id = ? AND client_code = ? AND deleted_at IS NULL AND status = 'active'")
      ...
      return json({ downloadUrl, expiresAt: new Date(Date.now() + ttl * 1000).toISOString() });
    }
    ...
    if (request.method === "DELETE" && deleteMatch) {
      ...
      await env.DB.batch([
        env.DB
          .prepare("UPDATE files SET deleted_at = ?, status = 'deleted' WHERE file_id = ?")
      ...
      ctx.waitUntil(fetch(deleteUrl, { method: "DELETE" }).catch(() => undefined));
